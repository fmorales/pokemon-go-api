...
def restartProcess():
    ...
    if accessToken is not None and globalltype is not None:
        ...
        access_token, ltype = main.get_acces_token(argsStored.username,
                                                   argsStored.password,
                                                   argsStored.type.lower())
...
def walk_random():
    LATITUDE, LONGITUDE, ALTITUDE = location.get_location_coords()
    LATITUDE = location.l2f(LATITUDE)
    LONGITUDE = location.l2f(LONGITUDE)
    ALTITUDE = location.l2f(ALTITUDE)
    LATITUDE += config.steps
    LONGITUDE += config.steps
    location.set_location_coords(LATITUDE, LONGITUDE, ALTITUDE)
